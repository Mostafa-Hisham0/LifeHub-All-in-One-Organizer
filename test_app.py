...
def update_habit():
    ...
    if request.method == 'POST':
        ...
        frequency = request.form.get("frequency")
        if not habit_name or not frequency:
            return render_template("update_habit.html", habit=habit, error="All fields are required.")
        update_habit_in_db(habit_id, {"name": habit_name, "frequency": frequency})
        return redirect(url_for("index"))
...
def increase_streak():
    ...
    if not habit_id:
        flash("Habit ID is required.", "danger")
        return redirect(url_for("index"))
    ...
    try:
        ...
        if last_updated:
            ...
            if habit["frequency"] == "daily" and now - last_updated < timedelta(days=1):
                flash("You can't increase streak yet. Try tomorrow.", "warning")
                return redirect(url_for("index"))
            if habit["frequency"] == "weekly" and now - last_updated < timedelta(weeks=1):
                flash("You can't increase streak yet. Try next week.", "warning")
                return redirect(url_for("index"))

        habit["streak"] += 1
        habit["last_updated"] = now.isoformat()
        update_habit_in_db(habit_id, {"streak": habit["streak"], "last_updated": habit["last_updated"]})
        flash("Streak increased successfully!", "success")
    except Exception as e:
        flash(f"Error updating streak: {e}", "danger")
...
def remove_habit():
    ...
    try:
        ...
        flash(f"Error removing habit: {e}", "danger")
